...
class GameResource(val service: GameService) {
    ...
    @CrossOrigin
    @PostMapping("/get")
    fun getCommand(@RequestBody mode: String) = service.getCommand(mode.removeQuotes().toMode())
    ...
    @CrossOrigin
    @PostMapping("/add")
    fun addCommand(@RequestBody command: Int) = service.addCommand(command.toCommand())
